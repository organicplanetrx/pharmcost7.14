...
import { RailwayDatabaseStorage } from './storage-database';



// Smart storage selection: use Railway Database if available, fallback to Memory
function createSmartStorage(): IStorage {
  const databaseUrl = process.env.DATABASE_URL;
  
  if (databaseUrl && databaseUrl.includes('postgresql://')) {
    console.log('🚂 Railway PostgreSQL detected - attempting database connection');
    ...
    console.log('💾 PostgreSQL not available - using memory storage');
